...
namespace Malee.Editor {
	...
	public class ReorderableList {
		...
		private float GetElementHeight(SerializedProperty element) {
			...
			else if (!ElementPassesFilter(element)) {
				return 0;
			}
			else {
		...
		*/
		...
		private void UpdateElementRects(Rect rect, Event evt) {
			...
			if (evt.type == EventType.Repaint) {
				...
				for (i = 0; i < elementRects.Length; i++) {

					SerializedProperty element = list.GetArrayElementAtIndex(i);

					if (ElementPassesFilter(element)) {
						...
						elementRects[i] = new Rect();
					}
				}
			}
		}

		private bool ElementPassesFilter(SerializedProperty element) {
			if (filterString != null && filterString.Length > 0) {
				if (null != getElementNameCallback) {
					return GetElementLabel(element, true).text.Contains(filterString);
				}
			}
			return true;
		}
		...
		private void DrawElements(Rect rect, Event evt) {
			...
			if (!dragging) {
				...
				for (int i = 0; i < list.arraySize; i++) {
					if (!ElementPassesFilter(list.GetArrayElementAtIndex(i)))
						continue;
		...
		private void HandleMoveElement(object userData) {
			...
			int size = elementRects.Length;
			int offset = toPage * size;
		...
		private void DoSelection(int index, bool setKeyboardControl, Event evt) {
			...
			if (draggable) {
				...
				UpdateDragList(dragPosition, 0, elementRects.Length);

				selection.Trim(0, elementRects.Length);
		...
		private int GetSelectionIndex(Vector2 position) {
			for (int i = 0; i < elementRects.Length; i++) {
				...
				if (rect.Contains(position) || (i == 0 && position.y <= rect.yMin) || (i == elementRects.Length - 1 && position.y >= rect.yMax)) {
